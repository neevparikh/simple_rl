...
class BeliefMDP(MDP):
    def __init__(self, pomdp):
        ...
        MDP.__init__(self, pomdp.actions, self._belief_transition_function,
                     self._belief_reward_function,
                     BeliefState(pomdp.init_belief), pomdp.gamma,
                     pomdp.step_cost)

    def _belief_transition_function(self, belief_state, action):
        '''
        The belief MDP transition function T(b, a) --> b' is a generative 
        function that given a belief state and an action taken from that belief 
        state, returns the most likely next belief state
        Args:
        ...
        next_belief_distribution = self.belief_updater_func(
            belief_state.distribution, action, observation)
        return BeliefState(next_belief_distribution)

    def _belief_reward_function(self, belief_state, action):
        '''
        The belief MDP reward function R(b, a) is the expected reward from the 
        POMDP reward function over the belief state distribution.
        Args:
            belief_state (BeliefState)
            action (str)
    ...
    def _get_observation_from_environment(self, action):
        ...
        Returns:
            observation (str): retrieve observation from underlying unobserved 
            state in the POMDP
        '''
        return self.state_observation_func(self.pomdp.cur_state, action)

    def execute_agent_action(self, action):
        reward, next_state = super(BeliefMDP,
